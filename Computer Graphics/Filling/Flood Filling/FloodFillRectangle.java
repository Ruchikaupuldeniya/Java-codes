...
public class FloodFillRectangle extends JFrame {

    public Color[][] pixels = new Color[600][600]; // Track colors of pixels
    ...
    public FloodFillRectangle() {
        setTitle("Flood Fill Algorithm Example");
        setSize(600, 600);
    ...
    public void paint(Graphics g) {
        ...
        fillRectangle(g, x1, y1, width, height, Color.RED);

        // Apply flood fill to change the interior color to blue
        floodFill(x1 + width / 2, y1 + height / 2, Color.RED, Color.BLUE, g);
    }

    public void drawRectangle(Graphics g, int x1, int y1, int width, int height) {
        drawLineDDA(g, x1, y1, x1 + width, y1); // Top
        drawLineDDA(g, x1, y1 + height, x1 + width, y1 + height); // Bottom
        drawLineDDA(g, x1, y1, x1, y1 + height); // Left
        drawLineDDA(g, x1 + width, y1, x1 + width, y1 + height); // Right
    }
    ...
    public void fillRectangle(Graphics g, int x1, int y1, int width, int height, Color color) {
        for (int x = x1 + 1; x < x1 + width; x++) {
            for (int y = y1 + 1; y < y1 + height; y++) {
                setPixelColor(g, x, y, color);
            }
        }
    }
    ...
    private void floodFill(int x, int y, Color targetColor, Color replacementColor, Graphics g) {
        ...
        if (currentColor != null && currentColor.equals(targetColor) && !currentColor.equals(replacementColor)) {
    ...
    public static void main(String[] args) {
        new FloodFillRectangle();
    }
}

/*
The issue arises because the flood-fill algorithm (floodFill) starts from a specific pixel 
and propagates outwards based on color matching. 
If the algorithm does not cover the entire rectangle, 
it could be due to the boundaries or the initial pixel chosen.
