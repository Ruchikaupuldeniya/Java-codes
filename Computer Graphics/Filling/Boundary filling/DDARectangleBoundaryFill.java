...
public class DDARectangleBoundaryFill extends JFrame {

    public Color[][] pixels = new Color[600][600]; // used to track the colors of individual pixels on the screen, allowing the program to store the state of each pixel
    int x1; // Top-left X coordinate
    int y1; // Top-left Y coordinate
    int width; // Width of the rectangle
    int height; // Height of the rectangle

    public DDARectangleBoundaryFill() {
        setTitle("DDA Rectangle Drawing with Boundary Fill");
        setSize(600, 600);
    ...
    public void paint(Graphics g) {
        ...
        // Start boundary fill from inside the rectangle (e.g., center)
        boundaryFill(x1 + width / 2, y1 + height / 2, Color.WHITE, Color.GREEN, g);
    }
